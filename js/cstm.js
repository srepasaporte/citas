...
$(document).on('change', '#numerocurps', function(e){
	...
	if(numero_curps.length > 0){
		...
		$(".nombre_curp").hide();
		$(".nombre_curp").val("");
		$(".nombre_curp").removeClass("not-valid curp-valid");

		for(i=0; i < numero_curps; i++){
			$("#numerocurp"+(i+1)).fadeIn().addClass('not-valid');
			$("#nombre_curp"+(i+1)).fadeIn().addClass('not-valid');
		}
	}else{
		$(".inputcurp").hide();
		$(".inputcurp").val("");
...
$(document).on('input', '.inputcurp', function(e){
	...
	if(texto_input.length == 18){
		...
		if(dataMensaje == 'OK'){
			...
			console.log("[Error en API] procede validaciÃ³n secundaria.")
			//validamos curp del input
		...
		//si falla...
			...
			console.log("Error al realizar la peticiÃ³n: (valida fallback)", error)
	...
	*/
	...
	function validateNextStep(){
		//si todos los curps son vÃ¡lidos y completos... paso siguiente
		if($(".inputcurp.curp-valid").length == cantidad_curps && $(".nombre_curp.curp-valid").length == cantidad_curps){
	...
	$(document).on('change', '.nombre_curp', function(e){
		var cantidad_curps = $('#numerocurps option:selected').val();

		var texto_input = $(this).val();
		var input = $(this);
		//CURP = 18 caract.
		if(texto_input.length > 0){
			console.log("VALIDAMOS NOMBRE CURPS: ", texto_input.length)
...
document.getElementById("resumenoficina2").value = $("p", ".oficinadetalle.ofinum"+oficina+" .card-body").html();
...
$(document).on('click', '.btn-confirmaoficina', function(e){
	...
	var futureBlockDate = new Date('2022-12-31');
...
//Progress Bar JS
function progressBar(percent,$element){var progressBarWidth=percent*$element.width()/100;$element.find('div').animate({width:progressBarWidth},1000).html("&nbsp;")}
